...
function SessionItem({
    ...
    return (
        ...
        >
                ...
                {isEditing ? (
                    ...
                    <>
                        <div className="flex items-center gap-1.5">
                            <p className={`truncate text-sm max-w-[110px] ${isActive && !isBatchMode ? 'font-medium text-foreground' : 'text-foreground/80'}`}>
                                {displayTitle}
                            </p>
                            {session.messageCount !== undefined && session.messageCount > 0 && (
                                <Badge variant="secondary" className="h-4 min-w-4 px-1 text-[10px] leading-none shrink-0">
            ...
            {!isBatchMode && (
                <div className={`flex flex-shrink-0 gap-0.5 w-[72px] justify-end transition-opacity ${isHovering && !isEditing ? 'opacity-100' : 'opacity-0'}`}>
                    <TagSelector
